...
const DotMatrix = ({
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      ...
      u_total_size: { value: 4, type: "uniform1f" },
  ...
  return (
    <Canvas className="absolute inset-0 h-full w-full">
      <ShaderMaterial source={fragmentShader(shaderCode)} uniforms={uniforms} />
    </Canvas>
  );
};
...
const ShaderMaterial = ({
  ...
  const shaderMaterial = useMemo(() => {
    ...
    for (const name in uniforms) {
      ...
      if (uniform.type.includes("3f")) {
        preparedUniforms[name] = {
          value: uniform.value.map((v: number[]) =>
            new THREE.Vector3().fromArray(v)
          ),
        };
      } else if (uniform.type.includes("1f") || uniform.type.includes("1fv")) {
...
const fragmentShader = (shaderLogic: string) => `
  precision mediump float;
  in vec2 fragCoord;
